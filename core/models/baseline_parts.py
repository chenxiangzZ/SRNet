...
from .utils import *
from .se_module import DRSLayer, MultiScaleLayer_v2, SELayer_Local, BasicConv2d
...
class Baseline(nn.Module):
    ...
    def forward(self, x, label=None):
        ...
        if self.training:
            ...
            lf_xent.extend([cls_score, cls_score_br1, cls_score_br2]*weights) #MMNet-a loss

            return x, tuple(lf_xent), tuple(local_feat)
        else:
            # return feat
            local_feat.append(feat)
            return x, torch.cat(local_feat, dim=1)
