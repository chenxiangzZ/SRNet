...
class SELayer2GM(nn.Module):
    def __init__(self, channel, reduction=1, ft_flag=True):
        super(SELayer2GM, self).__init__()
        self.ft_flag = ft_flag
        # 这里由于是特征向量，修改为1d
...
class DRSLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        b, c, _, _ = x.size()
        abs_x = torch.abs(x)
        abs_mean = self.avg_pool(abs_x).view(b, c)
        y = self.fc(abs_mean).view(b, c, 1, 1)
        thres = abs_mean.view(b, c, 1, 1).expand_as(x) * y.expand_as(x)
        # soft thresholding
        zero_t = torch.zeros(x.size()).cuda()
        d = torch.max(abs_x - thres, zero_t)
        b = torch.sign(x)
        return  d*b

# DRSN 深度残差收缩网络
class DRSLayer2GM(nn.Module):
    def __init__(self, channel, reduction=1):
        super(DRSLayer2GM, self).__init__()
    ...
    def forward(self, x):
        ...
        abs_x = torch.abs(x)
        abs_mean = self.avg_pool(abs_x).view(b, c)
        y = self.fc(abs_mean).view(b, c, 1)
        thres = abs_mean.view(b, c, 1).expand_as(x) * y.expand_as(x)
        # soft thresholding
        zero_t = torch.zeros(x.size()).cuda()
        d = torch.max(abs_x - thres, zero_t)
        b = torch.sign(x)
