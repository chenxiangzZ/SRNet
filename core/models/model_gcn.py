...
class AdaptDirGraphGonvLayer(nn.Module):
    ...
    def learn_adj(self, inputs, adj):
        ...
        # inputs [bs, k(node_num), c] = [128, 14, 2048])
        bs, k, c = inputs.shape

        global_features = inputs[:, k - 1, :].unsqueeze(1).repeat([1, k, 1])  # [bs,k,2048]
        # 13个局部特征的2048维向量与对应的全局特征进行绝对值相减
